...
const Logo = ({ setIsExpanded, isExpanded }: Props) => {
  return (
    <div className=" flex flex-row gap-2 items-center justify-center p-2 select-none">
      ...
      >
        <div
          className={`text-white font-secondaryRegular text-3xl whitespace-nowrap ml-4 transition-all duration-300 ease-in-out transform ${isExpanded ? "opacity-100 translate-x-0" : "opacity-0 -translate-x-4 pointer-events-none"}`}
        >
          NoteTube
        </div>
      </div>
